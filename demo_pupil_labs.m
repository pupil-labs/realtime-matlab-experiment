...
function [] = demo_pupil_labs()
sca;
close all;
clear;

% Check status of eyetracker server
r = pupil_labs_realtime_api();
if r.StatusCode ~= 200
    error(['Error connecting to Pupil Eyetracker, ensure the device is connected ',...
        'to the same network as this PC, and that HTTP ports are not blocked.\n', ...
        'You can also check in the browser if http://pi.local:8080/ does connect to your device.']);
end
...
try
    ...
    r = pupil_labs_realtime_api('Command','save');
    cleanup;

catch lasterror
    cleanup;
    % If an error ocurred, discard recording
    pupil_labs_realtime_api('Command','cancel');
end

function cleanup
    % Close window:
    RestrictKeysForKbCheck([]); FlushEvents;
    sca;
    Priority(0);
    ShowCursor;
    try
